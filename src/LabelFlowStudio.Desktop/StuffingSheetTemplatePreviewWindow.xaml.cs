...
public partial class StuffingSheetTemplatePreviewWindow : Window
{
    ...
    private static readonly TemplateToken[] Tokens =
    {
        ...
        new("{{Lndnam}}", "{{Lndnam}}", "Страна"),
        new("{{Tenam}}", "{{Tenam}}", "TENAM"),
        new("{{Market}}", "{{Market}}", "Маркет"),
        new("{{Gpplz}}", "{{Gpplz}}", "Индекс"),
        new("{{Gpbez}}", "{{Gpbez}}", "Грузополучатель"),
        new("{{Gport1}}", "{{Gport1}}", "Город"),
        new("{{Gpstrasse}}", "{{Gpstrasse}}", "Улица"),
        ...
        new("{{Artbez}}", "{{Artbez}}", "Наименование"),
        new("{{Bstmg}}", "{{Bstmg}}", "Количество"),
    };
    ...
    private async void OnLoaded(object sender, RoutedEventArgs eventArgs)
    {
        ...
        try
        {
            ...
            _templateText = await StuffingSheetTemplateStore.LoadOrCreateAsync(CancellationToken.None);
    ...
    private async Task RenderPreviewAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var html = StuffingSheetHtmlTemplateRenderer.Render(_templateText, _response, _tenam);
    ...
    private async Task<bool> SaveTemplateAsync()
    {
        ...
        try
        {
            await StuffingSheetTemplateStore.SaveAsync(_templateText, CancellationToken.None);
    ...
    {
        Title = _isDirty ? "Лист сброса *" : "Лист сброса";
    }
    ...
    private static string GetUiStatePath()
    {
        ...
        return Path.Combine(dir, "stuffingsheet-editor-state.json");
    }
